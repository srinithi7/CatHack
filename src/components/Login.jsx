    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-[#F3F3EF] px-4 py-10">
      ...
      >
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-[#FFCD11]">[SYSTEM_NAME]</h1>
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <LoginButton
            icon={Truck}
            label="Login as Renter"
            variant="outline"
            onClick={() => onLogin("renter")}
          />
          <LoginButton
            icon={User}
      ...
      <p className="mt-8 text-xs text-[#6E6B62] text-center">
        © 2025 [SYSTEM_NAME] | Powered by Caterpillar
      </p>
    </div>
  );
}

function LoginButton({ icon: Icon, label, variant, onClick }) {
  const base =
    "group w-full flex items-center justify-center gap-2.5 rounded-xl px-5 py-3.5 font-semibold text-sm transition-all duration-200 cursor-pointer";
